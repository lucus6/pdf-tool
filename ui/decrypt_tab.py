...
class DecryptTab(BaseTab):
    def __init__(self):
        ...
        self.input_password.setPlaceholderText("输入 PDF 打开密码")
        ...
        self.btn_decrypt = QPushButton("解密 PDF")
        self.btn_decrypt.setMinimumHeight(36)
        self.btn_decrypt.setStyleSheet("QPushButton { font-size: 14px; }")
    ...
    def _set_input(self, path):
        ...
        try:
            ...
            encrypted = "(加密) " if reader.is_encrypted else ""
            self.lbl_file.setText(f"{encrypted}{os.path.basename(path)} （{count} 页）")
            self._update_button()
    ...
    def _update_button(self):
        ok = bool(self.input_path and self.output_dir and self.input_password.text())
        self.btn_decrypt.setEnabled(ok)

    def _run_decrypt(self):
        pwd = self.input_password.text()

        def _done(path):
            self.lbl_status.setText(f"解密完成：{os.path.basename(path)}")
            self._update_button()

        self._start_worker(
            engine.decrypt_pdf, (self.input_path, pwd, self.output_dir), _done,
            disable_btn=self.btn_decrypt, status_label=self.lbl_status
        )
