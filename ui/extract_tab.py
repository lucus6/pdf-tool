...
class ExtractTab(BaseTab):
    def __init__(self):
        ...
        row2 = QHBoxLayout()
        row2.addWidget(QLabel("要提取的页码（例: 2,5,7）："))
        ...
        self.btn_extract = QPushButton("提取页面")
        self.btn_extract.setMinimumHeight(36)
        self.btn_extract.setStyleSheet("QPushButton { font-size: 14px; }")
    ...
    def _update_button(self):
        ...
        if ok:
            try:
                ...
                ok = False
        self.btn_extract.setEnabled(ok)
    ...
    def _run_extract(self):
        ...
        def _done(path):
            self.lbl_status.setText(f"提取完成：{os.path.basename(path)} （{engine.get_page_count(path)} 页）")
            self._update_button()

        self._start_worker(
            engine.extract_pages, (self.input_path, pages, self.output_dir), _done,
            disable_btn=self.btn_extract, status_label=self.lbl_status
        )
